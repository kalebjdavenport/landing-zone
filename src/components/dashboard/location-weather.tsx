...
export function LocationWeatherCard({ weather }: { weather: LocationWeather | null }) {
  if (!weather) {
    return (
      <Card>
        <CardHeader>
    ...
    <Card>
      <CardHeader>
        <CardTitle className="inline-flex items-center gap-2">
          ...
          {weather.point.city}, {weather.point.state}
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
          ...
          <div className="rounded-md border border-slate-200 p-2">
            ...
            <div className="mt-1 inline-flex items-center gap-1 text-sm font-semibold text-slate-900">
              ...
              {weather.current.textDescription ?? "Unknown"}
            </div>
          </div>
        </div>
        <Separator />
        ...
        <Separator />
        <div>
          <h4 className="text-sm font-semibold text-slate-900">Next periods</h4>
          <div className="mt-2 grid grid-cols-1 gap-2 md:grid-cols-2">
            {weather.forecast.slice(0, 4).map((period) => (
              <div key={period.startTime} className="rounded-md border border-slate-200 p-2 text-sm">
                <div className="font-medium text-slate-900">{period.shortForecast}</div>
